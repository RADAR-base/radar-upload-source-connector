...
class AltoidaHeaderProcessor : CsvHeaderProcessorFactory {
    ...
    override val fileHeader = listOf(
            "LABEL",
            "TIMESTAMP",
            "CLASS",
            "NMI",
    ...
    override fun process(header: List<String>): List<String> {
        val nonEmptyHeaders = header.filter { h -> h.isNotEmpty() }
        return if (nonEmptyHeaders.size < fileHeader.size) fileHeader else header
    }
