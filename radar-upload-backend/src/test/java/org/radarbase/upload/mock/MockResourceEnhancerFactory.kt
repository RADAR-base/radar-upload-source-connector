...
class MockResourceEnhancerFactory(
    ...
    override fun createEnhancers(): List<JerseyResourceEnhancer> = listOf(
            ...
            RadarJerseyResourceEnhancer(AuthConfig(jwtResourceName = config.jwtResourceName)),
            object : JerseyResourceEnhancer {
                override fun enhanceBinder(binder: AbstractBinder) {
                    val projectService = MockProjectService()
                    binder.bind(projectService)
                            .to(ProjectService::class.java)

                    binder.bind(projectService)
                            .to(UploadProjectService::class.java)

                    binder.bind(MockAuthValidator())
